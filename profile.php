...
<style>
    ...
    <body class="bg-dark text-light">
        ...
        <div><a href="index.php"><img src="logo_white.png"></a></div>
         <?//TODO Сделать нормальное отображение элементов (чтобы <select> нормально распологался, желательно без width)
                    if($user->Avatar){
                        //echo("<img id='ava' src = 'data:image/png;base64,".base64_encode($user->Avatar)."'>");
                        echo("
                        <img class=mb-3 style='align-self:center;' id= 'ava' src = ".$user->Avatar.">
                        <form method='post'>
            ...
            <p> 
                ...
                <?
                    ...
                    $games = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC);
                    //$genres = array();
                    //$stmt = "SELECT Genre_Id FROM GenreGames WHERE Game_Id = ".$g['Id'];                        
                ...
                <select>
                    ...
                    foreach($games as $g){
                        //$g = ;
                        echo("<option value = ".get_game($g)->Name."</option>");
                    }
                    ?>
